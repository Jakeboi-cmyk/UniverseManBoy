...
function chooseAuthority(room, preferred = null) {
  if (!room.started) return room.authorityId;
  const current = room.players.get(room.authorityId);
  if (current && current.visible && current.ws.readyState === current.ws.OPEN) return room.authorityId;
  const candidates = [...room.players.values()]
    .filter(p => p.visible && p.ws.readyState === p.ws.OPEN)
    .sort((a,b) => a.id - b.id);
  if (!candidates.length) return room.authorityId;
  const next = preferred != null ? candidates.find(p => p.id === preferred)?.id : candidates[0].id;
  return next == null ? candidates[0].id : next;
}
function setAuthority(room, nextId) {
  if (nextId == null || !room.players.has(nextId) || room.authorityId === nextId) return;
  room.authorityId = nextId;
  const msg = { t: 'authorityChanged', authorityId: nextId, state: room.lastState };
  broadcast(room, msg);
}
function electAuthority(room) {
  const next = chooseAuthority(room);
  if (next !== room.authorityId) setAuthority(room, next);
}
function closePlayer(room, playerId) {
  ...
  if (room.started) {
    if (playerId === room.authorityId) {
      const next = chooseAuthority(room);
      if (room.players.size && next !== playerId) {
        room.authorityId = next;
        broadcast(room, { t: 'playerLeft', id: playerId });
        broadcast(room, { t: 'authorityChanged', authorityId: next, state: room.lastState });
        return;
      }
    }
    broadcast(room, { t: 'playerLeft', id: playerId });
  ...
  if (playerId === 0) {
    ...
    for (const p of room.players.values()) { try { p.ws.close(); } catch (_) {} }
...
wss.on('connection', ws => {
  ...
  ws.on('message', raw => {
    ...
    if (msg.t === 'createRoom') {
      if (room) return;
      const code = makeCode();
      room = { code, started: false, authorityId: 0, lastState: null, players: new Map() };
      const player = { id: 0, ws, name: cleanName(msg.profile?.name), color: cleanColor(msg.profile?.color), visible: true };
    ...
    if (msg.t === 'joinRoom') {
      ...
      room = target;
      playerId = id;
      const player = { id, ws, name: cleanName(msg.profile?.name), color: cleanColor(msg.profile?.color), visible: true };
    ...
    if (msg.t === 'visibility') {
      me.visible = !msg.hidden;
      if (room.started && playerId === room.authorityId && msg.hidden) electAuthority(room);
      else if (room.started && playerId !== room.authorityId && !msg.hidden && (!room.players.get(room.authorityId)?.visible)) electAuthority(room);
      return;
    }

    if (msg.t === 'input') {
      if (playerId === room.authorityId) return;
      const authority = room.players.get(room.authorityId);
      if (authority) send(authority.ws, { ...msg, id: playerId, t: 'input' });
      return;
    }

    if (msg.t === 'state') {
      if (playerId !== room.authorityId) return;
      room.lastState = msg.s || null;
